...
async def main(collect: bool, country: str, league: str, season: str | None = None) -> list:
    ...
    async with async_playwright() as pw:
        ...
        # await page.get_by_label("Username").fill("cvetaneee")
        # await page.get_by_label("Password").fill(os.environ.get("ODDSPORTAL_PASSWORD"))
        # await page.get_by_role("button", name="Login").click()
        # await page.wait_for_timeout(2000)  # wait for login to settle

        matches = []
        if type == "historic":
            url = f"https://www.oddsportal.com/football/{country}/{league}-{season}/results/"
